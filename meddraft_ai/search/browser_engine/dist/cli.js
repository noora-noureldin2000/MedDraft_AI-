...
async function runCommand(command, args, browser, browserConfig, setPage) {
    ...
    if (command === 'scholar-search') {
        ...
        if (!url || !agentId)
            throw new Error('Usage: click <url> <agentId>');
        await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
        await browser.delay(2000, 3000);
        await browser.solveReCaptcha(page);
        ...
        if (!pdfUrl || !outputPath)
            throw new Error('Usage: download-pdf <pdfUrl> <outputPath>');
