...
export interface PubMedBrowserResult {
  index: number;
  pmid: string;
  title: string;
  snippet: string;
...
export class PubMedSurfer {
  ...
  async search(page: Page, query: string, limit = 10): Promise<PubMedBrowserResult[]> {
    ...
    if (isSingleResult) {
      ...
      const pdfLink = await page.$eval('.full-text-links a', el => (el as HTMLAnchorElement).href).catch(() => null);
