...
import { BrowserConfig, HumanBrowser } from './human_browser.js';
import { PageController, PageSnapshot } from './page_controller.js';
...
function optionalIndex(args: Record<string, unknown>, key: string, fallback: number): number {
  const val = args[key];
...
export async function runSession(config: BrowserConfig): Promise<void> {
  ...
  async function ensureContext(): Promise<BrowserContext> {
    if (!context) context = await human.launch();
    return context;
  }

  async function bindActivePage(): Promise<Page> {
    ...
    if (controller.boundTo() !== page) controller.attach(page);
    return page;
  }

  async function snapshotOrThrow(): Promise<PageSnapshot> {
  ...
  const handlers: Record<string, Handler> = {
    ...
    click: async (args) => {
      ...
      if (index < 0) throw new Error('Missing or invalid argument "index" (expected non-negative integer).');
    ...
    type: async (args) => {
      ...
      if (text.length > 500) throw new Error('Argument "text" exceeds 500 characters.');
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      const payload: Record<string, unknown> = { id, ok: true };
