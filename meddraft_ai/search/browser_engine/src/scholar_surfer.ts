...
import { cleanScientificDOM, CompressedNode } from './dom_compressor.js';
...
export interface ScholarResult {
  ...
  titleAgentId: number | null;
  pdfAgentId: number | null;
}

export class ScholarSurfer {
  private browser: HumanBrowser;

  constructor(browser: HumanBrowser) {
    this.browser = browser;
  }

  async search(page: Page, query: string, limit = 10): Promise<ScholarResult[]> {
    ...
    const results = await page.evaluate(() => {
      ...
      return Array.from(items).map((el, idx) => {
        ...
        let pdfAgentId: number | null = null;
        ...
        if (authorParts.length > 0) authors = authorParts[0].trim();
        ...
        links.forEach((link) => {
          ...
          if (text.includes('Cited by')) {
            ...
            if (citeMatch) citations = parseInt(citeMatch[0], 10);
          }
        });
        ...
        return {
          ...
          year,
          citations,
          url: (titleEl as HTMLAnchorElement)?.href || null,
          pdfLink: (pdfEl as HTMLAnchorElement)?.href || null,
          hasDirectPDF: !!pdfEl,
          titleAgentId,
          pdfAgentId
        };
