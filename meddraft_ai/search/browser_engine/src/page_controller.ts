...
export interface PageSnapshot {
  ...
  scroll: { abovePx: number; belowPx: number };
...
function elementKey(e: RawElement): string {
...
export class PageController {
  ...
  private pageRef(): Page {
    if (!this.page) throw new Error('No page attached to controller.');
    return this.page;
  }
  ...
  async snapshot(): Promise<PageSnapshot> {
    ...
    const raw: RawElement[] = await page.evaluate((): RawElement[] => {
      ...
      const indexed = new WeakSet<Element>();
      const hasIndexedAncestor = (el: Element): boolean => {
        let cur = el.parentElement;
        while (cur) {
          if (indexed.has(cur)) return true;
      ...
      for (const el of Array.from(document.querySelectorAll('*'))) {
        ...
        if (!isCoreTag && !INTERACTIVE_ROLES.has(role) && hasIndexedAncestor(el)) continue;
        ...
        for (const name of ATTR_WHITELIST) {
          ...
          if (val) attrs[name] = val.replace(/\s+/g, ' ').slice(0, 100);
        }
        const text = ((el as HTMLElement).innerText || el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 200);
        out.push({
          index,
          tag: tagLower,
          text,
          href: tagLower === 'a' ? (el as HTMLAnchorElement).href || null : null,
          attrs,
        });
      }
      return out;
    ...
    const elements: SnapshotElement[] = raw.map((e) => ({
      ...e,
  ...
  async click(index: number): Promise<{ clicked: string; newTabIndex: number | null }> {
    const page = this.pageRef();
  ...
  async type(index: number, text: string, submit: boolean): Promise<void> {
    const page = this.pageRef();
    const locator = page.locator(`[data-agent-index="${index}"]`);
    if ((await locator.count()) === 0) {
      throw new Error(`No element for index ${index} — the page changed since the last snapshot. Call state.`);
  ...
  async scroll(down: boolean, pixels: number): Promise<void> {
    const page = this.pageRef();
    await page.mouse.wheel(0, down ? pixels : -pixels);
  }

  async settle(minMs = 1200, maxMs = 2800): Promise<void> {
