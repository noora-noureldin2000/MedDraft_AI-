...
A comprehensive guide to styling and customizing Matplotlib visualizations.
...
Best for ordered data that varies from low to high.
...
**3. Diverging**
Best for data with a significant center point (e.g., zero, mean).
...
**Usage:**
```python
# Set colormap center to zero
im = ax.imshow(data, cmap='coolwarm', vmin=-1, vmax=1)
```

**4. Qualitative**
Best for categorical/nominal data without inherent order.
...
**5. Cyclic**
Best for cyclic data (e.g., phase, angles).
- `twilight`
- `twilight_shifted`
- `hsv`
...
1. **Avoid using `jet` colormap** - Not perceptually uniform, can be misleading.
2. **Use perceptually uniform colormaps** - Such as `viridis`, `plasma`, `cividis`.
3. **Consider colorblind users** - Use `viridis`, `cividis` or test with a colorblind simulator.
4. **Match colormap to data type**:
   - Sequential: increasing/decreasing data
   - Diverging: data with significant center
...
# Create discrete colormap from continuous colormap
cmap = plt.cm.viridis
...
# Apply multiple styles (later styles override earlier)
...
```python
# Get evenly spaced colors from colormap
n_colors = 5
...
ax.grid(True, axis='x')  # Vertical lines only
ax.grid(True, axis='y')  # Horizontal lines only

# Set grid below or above data
...
# Precise position (bbox_to_anchor)
...
ax.legend(
    ...
    facecolor='white'       # Border background color
)
```
...
```python
# Recommended method (auto-adjust)
...
## Professional Publication Styles

Configuration example for publication-quality figures:
...
# Use simulation tools to test or use these validated palettes
```
