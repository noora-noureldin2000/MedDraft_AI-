...
This guide addresses frequently encountered issues when using matplotlib.

## Display and Backend Issues

### Issue: Figure Not Displaying

**Symptoms:** Nothing happens after calling `plt.show()`.

**Solutions:**
```python
# 1. Check if backend is set correctly (for interactive use)
import matplotlib
...
# 3. In Jupyter notebook, use magic commands
%matplotlib inline  # static images
# or
%matplotlib widget  # interactive plots

# 4. Make sure to call plt.show()
plt.plot([1, 2, 3])
plt.show()
```
...
**Symptoms:** Interactive mode issues in multithreaded environment.
...
### Issue: Interactive Mode Not Updating Plot

**Symptoms:** Changes to the figure are not reflected in the interactive window.
...
### Issue: Labels and Titles Overlapping

**Symptoms:** Labels, titles, or tick labels overlap or get clipped.
...
# Solution 3: Manual margin adjustment
plt.subplots_adjust(left=0.15, right=0.95, top=0.95, bottom=0.15)

# Solution 4: Use bbox_inches='tight' when saving
...
### Issue: Colorbar Affecting Subplot Size

**Symptoms:** Adding a colorbar shrinks the original plot.
...
# Solution 2: Manually specify colorbar size
from mpl_toolkits.axes_grid1 import make_axes_locatable
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
plt.colorbar(im, cax=cax)

# Solution 3: Shared colorbar for multiple subplots
...
### Issue: Subplots Too Crowded

**Symptoms:** Multiple subplots overlap.
...
# Solution 2: Use subplots_adjust to adjust spacing
...
**Symptoms:** Memory usage keeps growing when creating many figures.
...
**Symptoms:** Saved image files are too large.
...
**Symptoms:** Plotting large number of points takes too long.

**Solutions:**
```python
# Solution 1: Downsample data
from scipy.signal import decimate
y_downsampled = decimate(y, 10)  # Take every 10th point
...
**Symptoms:** "findfont: Font family [...] not found" warnings.
...
**Symptoms:** Math text not rendering correctly.

**Solutions:**
```python
# Solution 1: Use raw string with r prefix
ax.set_xlabel(r'$\alpha$')  # instead of '\alpha'
...
# Mathtext is always available without LaTeX installation
ax.text(x, y, r'$\int_0^\infty e^{-x} dx$')
```

### Issue: Text Clipped or Outside Figure Bounds

**Symptoms:** Labels or annotations appear outside figure edges.
...
# Solution 4: Use constrained layout
fig, ax = plt.subplots(constrained_layout=True)
```
...
**Symptoms:** Colorbar shows different range than the data.
...
# Or use same normalization for multiple plots
...
### Issue: Colors Displaying Incorrectly

**Symptoms:** Unexpected colors appear in the plot.
...
# Solution 3: For scatter plots, ensure c parameter shape matches
ax.scatter(x, y, c=colors)  # colors length should match x, y

# Solution 4: Check alpha (transparency) setting
ax.plot(x, y, alpha=1.0)  # 0=transparent, 1=opaque
```

### Issue: Reversed Colormap

**Symptoms:** Colormap direction is opposite to expected.

**Solutions:**
```python
# Add _r suffix to any colormap name to reverse it
...
**Symptoms:** `set_xlim` or `set_ylim` has no effect.
...
### Issue: Log Scale Axis with Zero or Negative Values

**Symptoms:** Using log scale throws ValueError when data ≤ 0.
...
**Symptoms:** Date axis shows numbers instead of date format.
...
**Solutions:**
```python
# Solution 1: Use 'best' auto-positioning
ax.legend(loc='best')
...
# Solution 4: Place legend below figure
ax.legend(bbox_to_anchor=(0.5, -0.15), loc='upper center', ncol=3)
```

### Issue: Too Many Legend Entries

**Symptoms:** Legend becomes cluttered with too many entries.

**Solutions:**
```python
# Solution 1: Only add labels for selected items
...
# Solution 3: Create simplified custom legend
...
**Symptoms:** Hard to perceive depth in 3D plots.
...
# Solution 2: Add grid lines
ax.grid(True)

# Solution 3: Use color to represent depth
scatter = ax.scatter(x, y, z, c=z, cmap='viridis')

# Solution 4: Interactive rotation (if using interactive backend)
# User can click and drag to rotate
```

### Issue: 3D Axis Labels Clipped

**Symptoms:** 3D axis labels appear outside the figure.
...
# Or use tight bbox when saving
plt.savefig('3d_plot.png', bbox_inches='tight', pad_inches=0.5)
```

## Image and Colorbar Issues

### Issue: Image Displayed Upside Down

**Symptoms:** Image orientation is incorrect.
...
**Symptoms:** Image appears blocky when zoomed in.

**Solutions:**
```python
# Solution 1: Use interpolation
ax.imshow(img, interpolation='bilinear')
# Options include: 'nearest', 'bilinear', 'bicubic', 'spline16', 'spline36', etc.

# Solution 2: Increase DPI when saving
plt.savefig('figure.png', dpi=300)

# Solution 3: Use vector format if applicable
...
**Symptoms:** Trying to index a single axes object.
```python
# Wrong
fig, ax = plt.subplots()
ax[0].plot(x, y)  # Error!
...
**Symptoms:** Data array lengths don't match.
```python
# Check shapes
...
**Symptoms:** Calling plot on array instead of axes object.
```python
# Wrong
data.plot(x, y)
...
1. **Always use Object-Oriented (OO) interface** - Avoid pyplot state machine.
   ```python
   fig, ax = plt.subplots()  # Recommended
   ax.plot(x, y)
   ```

2. **Use constrained_layout** - Prevent overlap issues.
   ```python
   fig, ax = plt.subplots(constrained_layout=True)
   ```

3. **Explicitly close figures** - Prevent memory leaks.
   ```python
   plt.close(fig)
   ```
...
5. **Use raw strings for math text** - Avoid escape character issues.
   ```python
   ax.set_xlabel(r'$\alpha$')
   ```

6. **Check data shapes before plotting** - Catch dimension mismatches early.
   ```python
   assert len(x) == len(y)
   ```
...
8. **Test different backends when display issues occur**
   ```python
   import matplotlib
