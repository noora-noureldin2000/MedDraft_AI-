# Matplotlib API Reference Manual
...
**Key Methods:**
- `fig.add_subplot(nrows, ncols, index)` - Add subplot
...
- `fig.suptitle(title)` - Set figure-wide title
...
- `fig.dpi` - Dots per inch resolution
- `fig.figsize` - Figure size in inches (width, height)

### Axes

The actual plotting area for data visualization.
...
- `ax.errorbar(x, y, yerr, xerr)` - Error bar plot
...
- `ax.contour(X, Y, Z, levels=10)` - Contour plot
- `ax.contourf(X, Y, Z, levels=10)` - Filled contour plot
...
- `ax.annotate(text, xy=(x, y), xytext=(x2, y2), arrowprops={})` - Annotated text with arrow
...
- `ax.set_xscale('linear'/'log'/'symlog')` - Set x-axis scale type
- `ax.set_yscale('linear'/'log'/'symlog')` - Set y-axis scale type

**Ticks:**
- `ax.set_xticks(positions)` - Set x-axis tick positions
...
**Plotting (using current axes):**
- `plt.plot()` - Line plot
- `plt.scatter()` - Scatter plot
- `plt.bar()` - Bar chart
- `plt.hist()` - Histogram
- (All Axes methods are available)
...
ax.text(x, y, text,
    ...
    fontweight='normal',    # Weight: 'normal', 'bold', 'heavy', 'light'
    fontstyle='normal',     # Style: 'normal', 'italic', 'oblique'
...
ax.bar3d(x, y, z, dx, dy, dz)      # 3D bar chart
