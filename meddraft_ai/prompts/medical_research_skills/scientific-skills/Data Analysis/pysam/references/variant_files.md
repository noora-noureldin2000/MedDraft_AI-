...
## VariantFile Attributes

**Header Information:**
- `header` - Complete VCF header containing metadata
...
### Fetching Specific Regions

Requires tabix index (.tbi) for VCF.gz, or appropriate index for BCF:

```python
# Fetch variants in region (region string uses 1-based coordinates)
...
**Note:** Use **1-based coordinates** in `fetch()` calls to comply with VCF specification.
...
- `stop` - End position (0-based, exclusive)
- `id` - Variant ID (e.g., rsID)

### Allele Information
- `ref` - Reference genome allele
- `alts` - Tuple of variant alleles
...
Access INFO fields like a dictionary:
...
Access sample data through the `samples` dictionary:
...
```python
# Find variants where sample has variant allele
for variant in vcf:
    sample = variant.samples["sample1"]
    gt = sample["GT"]

    # Check if contains variant allele
...
### Extracting Variants for Specific Samples

```python
invcf = pysam.VariantFile("input.vcf")
samples_to_keep = ["sample1", "sample3"]

# Create new header with sample subset
...
### Converting VCF to Summary Table
...
1. **Use BCF format**, which has better compression and faster access than VCF.
2. **Use tabix index files** for efficient region queries.
...
1. **Coordinate system:** VCF uses 1-based coordinates, but `VariantRecord.start` is 0-based.
...
4. **Allele indices:** In genotype `(0, 1)`, 0=REF, 1=first ALT, 2=second ALT, etc.
5. **Index requirement:** Region-based `fetch()` requires tabix index for VCF.gz.
6. **Header modification:** When extracting sample subsets, properly update header and copy FORMAT fields.
