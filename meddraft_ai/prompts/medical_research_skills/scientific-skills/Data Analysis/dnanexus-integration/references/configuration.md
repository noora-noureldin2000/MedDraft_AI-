# DNAnexus App Configuration and Dependency Management

## Overview

This guide covers how to configure Apps via `dxapp.json` metadata, and how to manage dependencies including system packages, Python libraries, and Docker containers.
...
```json
{
  "name": "my-app",           // Unique identifier (lowercase letters, numbers, hyphens, underscores)
...
```json
{
  "version": "1.0.0",        // Semantic versioning (required for Apps)
  "description": "Extended description...",
  "developerNotes": "Implementation notes...",
  "categories": [            // Used for App discovery
    "Read Mapping",
    "Variation Calling"
  ],
...
- `hash` - Key-value pair mapping
- `array:class` - Array of specified type
...
- `patterns` - File name patterns (for file types)
- `suggestions` - Predefined reference data
- `choices` - Allowed values (for string/number)
- `group` - UI grouping
...
```json
{
  "runSpec": {
    ...
    "file": "src/my-app.py",         // Entry script
    "distribution": "Ubuntu",
    "release": "24.04",
...
Packages will be installed via `apt-get` from Ubuntu repositories.

### Python Dependencies

#### Option 1: Install pip via execDepends
...
Include custom tools or libraries in your App:
...
Assets are pre-built dependency packages that can be shared across multiple Apps.
...
Assets are mounted at runtime and accessed via environment variables:
```python
import os
...
```json
{
  "access": {
    ...
    "allProjects": "VIEW",      // View other projects
    "developer": true           // Elevated permissions
  }
}
```
...
- `["github.com", "pypi.org"]` - Specific domain access
...
1. **Versioning**: Use semantic versioning for Apps.
2. **Instance Types**: Start with smaller instances and scale up as needed.
3. **Dependencies**: Clearly document all dependencies.
...
7. **Resources**: Bundle commonly used tools to avoid repeated downloads.
8. **Docker**: Use Docker for complex dependency chains.
9. **Assets**: Create assets for heavy dependencies shared across multiple Apps.
10. **Timeouts**: Set reasonable timeout limits based on expected runtime.
11. **Network Access**: Only request necessary network permissions.
12. **Regional Support**: Use `regionalOptions` for multi-region Apps.
