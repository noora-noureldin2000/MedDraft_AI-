...
The dxpy library provides Python bindings for interacting with the DNAnexus platform. It can be used both in the DNAnexus execution environment (applications running on the platform) and in external scripts that access the API.
...
# Update details (must be in open state)
...
# Get job output reference (for job chaining)
...
# Must call at the end of app code
dxpy.run()
```
...
- `ResourceNotFound`: Object does not exist
...
1. **Use High-Level Functions**: Prefer `upload_local_file()` over manually creating files.
2. **Handler Reuse**: Create handlers once, reuse multiple times.
3. **Batch Operations**: Use find functions when handling multiple objects.
4. **Error Handling**: Always wrap API calls in try-except blocks.
5. **Close Objects**: Remember to close files and records after modification.
6. **Project Context**: Set workspace context for apps.
7. **API Token Security**: Never hardcode tokens in source code.
8. **Describe Fields**: Only request needed fields to reduce latency.
9. **Search Filters**: Use specific filters to narrow search results.
10. **Link Format**: Use `dxpy.dxlink()` to ensure consistent link creation.
...
# Job 2 depends on job 1's output
job2 = applet2.run({
