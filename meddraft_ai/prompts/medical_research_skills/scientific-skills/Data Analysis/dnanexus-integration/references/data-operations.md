...
DNAnexus provides comprehensive data management for files, records, databases, and other data objects. All data operations can be performed via the Python SDK (dxpy) or command-line interface (dx).
...
### Applets and Apps
Executable programs (introduced in app-development.md).
...
Most objects start in open state and need to be explicitly closed:
```python
# Close file
file_obj.close()
```
...
dxpy.api.project_invite(
    ...
    {
        "invitee": "user-xxxx",
        "level": "CONTRIBUTE"  # Options: VIEW, UPLOAD, CONTRIBUTE, ADMINISTER
    }
)
```
...
```python
# Remove file from project (not permanent delete)
...
Archived data is moved to lower-cost long-term storage:
...
2. **Use Properties**: Add meaningful property tags to data for easier search.
3. **Organize Folders**: Use clear, logical folder structures.
4. **Clean Up Data**: Remove temporary or obsolete data promptly.
5. **Batch Operations**: Use batch operations when processing large numbers of objects.
6. **Error Handling**: Check object state before performing operations.
7. **Permission Management**: Verify project permissions before data operations.
8. **Archive Old Data**: Use archiving to save on long-term storage costs.
