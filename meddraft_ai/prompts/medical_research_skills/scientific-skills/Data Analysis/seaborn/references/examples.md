...
This document provides practical examples of common data visualization scenarios using Seaborn.
...
# Pairwise relationships of all numerical variables
...
### Multi-panel Figure with Different Chart Types
...
# Right panel: Scatter + Violin plot
sns.violinplot(
    data=df,
    x='category',
    y='value',
...
```python
# Show changes in values across categories
sns.pointplot(
    data=df,
    x='timepoint',
    y='score',
...
### Linear Regression with Faceting

```python
# Fit independent regressions for each category
g = sns.lmplot(
    data=df,
    x='predictor',
    y='response',
...
# Overlay means with confidence intervals
sns.pointplot(
    data=dose_df,
    x='dose',
    y='response',
...
sns.set_theme(
    ...
    rc={
        ...
        'pdf.fonttype': 42,  # Use True Type fonts for PDF
    }
)
```
...
```python
# Suitable for data with a clear zero point (e.g., log fold change)
...
```python
# Smart sampling for extremely large datasets
