...
PathML provides comprehensive support for loading whole slide digital slides (WSI) from over 160 proprietary medical imaging formats. The framework abstracts vendor-specific complexities through unified slide classes and interfaces, enabling seamless access to image pyramids, metadata, and regions of interest (ROI) across different file formats.
...
- **Vectra** (`.qptiff`) - Multiplexed immunofluorescence
- **MERFISH** - Multiplexed error-robust fluorescence in situ hybridization

PathML automatically handles format-specific details using OpenSlide and other specialized libraries.

## Core Classes for Loading Images

### SlideData

`SlideData` is the fundamental class representing whole slide digital images in PathML.

**Loading from file:**
```python
from pathml.core import SlideData

# Load a whole slide digital image
wsi = SlideData.from_slide("path/to/slide.svs")

# Load using specific backend
...
- `wsi.level_dimensions` - Dimensions of each pyramid level
- `wsi.level_downsamples` - Downsampling factors for each pyramid level

**Methods:**
- `wsi.generate_tiles()` - Generate tiles from slide
- `wsi.read_region()` - Read specific region at specified level
- `wsi.get_thumbnail()` - Get thumbnail image

### SlideType

`SlideType` is an enumeration class defining supported slide backends:
...
SlideType.VectraQPTIFF  # For Vectra multiplexed immunofluorescence
```
...
# Load Vectra multiplexed immunofluorescence data
...
wsi.generate_tiles(
    ...
    tile_size=256,  # Tile pixel dimension
    stride=256,  # Tile stride (256 = no overlap)
...
```python
# Read region at specified location and level
...
Whole slide digital images are stored in multi-resolution pyramids. Select the appropriate level based on desired magnification:
...
Use `SlideDataset` to efficiently process multiple slides:
...
# Iterate through all tiles from all slides
for tile in dataset:
    image = tile.image
...
PathML provides specialized support for DICOM WSI:
...
OME-TIFF provides an open standard for multidimensional imaging:
...
# Access channel information for multi-channel image
...
For large WSI files (typically >1GB), use tile-based loading to avoid memory exhaustion:
...
- **Level 0:** For final analysis requiring maximum detail
- **Level 1-2:** For most preprocessing and model training
- **Level 3+:** For thumbnails, quality control, and rapid exploration

## Common Issues and Solutions

**Problem: Slide loading fails**
...
- Process at lower pyramid levels (e.g., level=1 or level=2)
- Reduce `tile_size` parameter
- Enable Dask distributed processing

**Problem: Color inconsistency between slides**
- Apply stain normalization preprocessing (see `preprocessing.md`)
- Check calibration information in scanner metadata
...
- Check available fields using `wsi.metadata`
- Some formats may have limited metadata support

## Best Practices

1. **Always check pyramid structure before processing:** View `level_dimensions` and `level_downsamples` to understand available resolutions.

2. **Use appropriate pyramid levels:** Most tasks can be processed at levels 1-2; reserve level 0 for final high-resolution analysis.

3. **Use overlapping tiles for segmentation tasks:** Set `stride < tile_size` to avoid edge artifacts.

4. **Verify magnification consistency:** When integrating slides from different sources, check `openslide.objective-power` metadata.

5. **Handle vendor-specific formats:** For multiparametric data, use specialized slide classes (CODEXSlide, VectraSlide).

6. **Implement quality control:** Generate thumbnails and check for artifacts before processing.

7. **Use distributed processing for large datasets:** Leverage Dask for parallel processing across multiple workers.

## Example Workflows

### Loading and Inspecting New Slides
...
# Generate thumbnail for quality control
thumbnail = wsi.get_thumbnail(size=(1024, 1024))
plt.imshow(thumbnail)
...
# Run distributed processing pipeline
dataset.run(pipeline, distributed=True, n_workers=8)
...
pipeline = Pipeline([
    CollapseRunsCODEX(z_slice=2),  # Select z-axis slice
    SegmentMIF(
...
- **OpenSlide:** https://openslide.org/ (Foundation library for WSI formats)
