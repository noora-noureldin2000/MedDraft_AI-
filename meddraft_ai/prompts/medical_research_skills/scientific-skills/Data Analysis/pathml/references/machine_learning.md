...
PathML provides comprehensive machine learning capabilities for computational pathology, including pre-built models for nuclei detection and segmentation, training workflows integrated with PyTorch, access to public datasets, and ONNX-based inference deployment. The framework seamlessly bridges image preprocessing with deep learning to enable end-to-end pathology machine learning pipelines.

## Pre-built Models

PathML includes state-of-the-art pre-trained models for nuclei analysis:

### HoVer-Net

**HoVer-Net** (Horizontal and Vertical Network) performs simultaneous nuclei instance segmentation and classification.

**Architecture:**
- Encoder-decoder structure with three prediction branches:
  - **Nuclear Pixel (NP)** - Binary segmentation of nuclear regions
  - **Horizontal-Vertical (HV)** - Distance map to nuclei centroids
  - **Classification (NC)** - Nuclei type classification
...
model = HoVerNet(
    num_types=5,  # Number of nuclei types
...
# - output['hv']: Horizontal-vertical map
...
# type_map: Each nucleus is assigned a type (1-5)
```

### HACTNet

**HACTNet** (Hierarchical Cell-Type Network) performs hierarchical nuclei classification with uncertainty quantification.
...
# Inference
output = model(tile_image)
coarse_pred = output['coarse']  # Coarse class
...
PathML models integrate with PyTorch Lightning to simplify training:
...
**PanNuke** contains 7,901 histology image patches from 19 tissue types with nuclei annotations for 5 cell types.
...
pannuke = PanNukeDataModule(
    ...
    batch_size=16,
    num_workers=4,
    tissue_types=None,  # Use all tissue types, or specify a list
...
for batch in train_loader:
    ...
    hv_map = batch['hv_map']  # Horizontal-vertical distance map
...
Access The Cancer Genome Atlas dataset:
...
Evaluate model performance using pathology-specific metrics:
...
Deploy models for production inference using ONNX:
...
# Fine-tune only decoder and classification head
optimizer = torch.optim.Adam(
...
1. **Use pre-trained models when possible:**
...
2. **Apply appropriate data augmentation:**
   ...
   - Color jitter to handle stain variation.
   - Elastic transform to account for biological variability.
...
4. **Handle class imbalance:**
   - Use weighted loss functions for rare cell types.
   - Oversample minority classes.
   - Use Focal Loss for hard examples.

5. **Validate on different tissue types:**
   - Ensure model generalization across tissues.
   - Test on held-out anatomical sites.

6. **Optimize inference:**
   - Export to ONNX for faster deployment.
   - Batch tile processing for better GPU utilization.
   - Use mixed precision (FP16) when possible.

7. **Save checkpoints regularly:**
   - Save best model based on validation metrics.
   - Save optimizer state for training recovery.

## Common Issues and Solutions

**Problem: Poor nuclei boundary segmentation**
- Use HV map (horizontal-vertical) to separate touching nuclei.
- Increase weight for HV loss term.
- Apply morphological post-processing.

**Problem: Similar cell types misclassification**
- Increase classification loss weight.
- Add hierarchical classification (HACTNet).
- Augment training data for confusing classes.

**Problem: Training unstable or not converging**
...
**Problem: Model overfitting training data**
