...
PathML provides tools for constructing spatial graphs from tissue images to represent cell-level and tissue-level relationships. Graph-based representations enable complex spatial analysis including neighborhood analysis, cell-cell interaction studies, and graph neural network (GNN) applications. These graphs capture morphological features and spatial topology for downstream computational analysis.

## Graph Types

PathML supports constructing multiple types of graphs:
...
- Suitable for tissue-level spatial pattern analysis

### Spatial Transcriptomics Graphs
- Nodes represent spatial locations (spots) or cells
...
### From Segmentation to Graph

Convert nuclei or cell segmentation results to spatial graphs:
...
graph = CellGraph.from_instance_map(
    inst_map,
    connectivity='radius',
    radius=100,  # Maximum pixel distance
    distance_metric='euclidean'  # or 'manhattan', 'chebyshev'
)
```
- Density-based variable node degree
...
- No isolated nodes (within convex hull)
- Captures spatial tessellation structure

**Contact-based:**
```python
# Connect cells that share boundary contact
graph = CellGraph.from_instance_map(
    inst_map,
    connectivity='contact',
    dilation=2  # Dilate boundaries to capture close contact
)
```
- Physical cell-cell contact
- Most direct biological interpretation
- Sparser edges for separated cells
...
morphology_features = extract_morphology_features(
    ...
    features=[
        ...
        'eccentricity',  # Shape eccentricity/stretch
...
- **Major/Minor axis** - Length of fitted ellipse axes
...
# Extract mean marker intensity for each cell
...
Add classification-derived cell type labels:
...
Aggregate features across the entire graph:
...
y = pyg_data.y  # Graph-level labels
...
# Create graph dataset from multiple slides
graphs = []
for slide in slides:
    # Construct graph for each slide
...
**Large tissue slides:**
- Construct graphs tile-by-tile, then merge
...
**Problem: Too many or too few edges**
...
- Keep only essential edge features

**Problem: Missing cells at tissue boundaries**
- Apply `edge_correction` parameter
- Use tissue masks to exclude invalid regions

**Problem: Inconsistent feature scales**
