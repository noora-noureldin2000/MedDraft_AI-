...
PathML provides specialized support for multiparametric imaging technologies that can simultaneously measure multiple markers at single-cell resolution. These technologies include CODEX, Vectra multiplexed immunofluorescence, MERFISH, and other spatial proteomics and transcriptomics platforms. PathML handles the unique data structures, processing requirements, and quantification workflows specific to each technology.

## Supported Technologies

### CODEX (CO-Detection by indEXing)
- Cyclic immunofluorescence imaging
- Simultaneous detection of 40+ protein markers
- Single-cell spatial proteomics
- Multi-cycle acquisition using antibody barcodes

### Vectra Polaris
- Multiplexed spectral immunofluorescence
- 6-8 markers per section
- Spectral unmixing
...
CODEX data is typically organized as multi-channel image stacks from multiple acquisition cycles:
...
codex_pipeline = Pipeline([
    ...
    CollapseRunsCODEX(
        ...
        run_order=None,  # Auto-detect cycle order, or specify [0, 1, 2, ...]
        method='max'  # Use 'max', 'mean', or 'median' across cycles
    ),

    # 2. Cell segmentation using Mesmer
    SegmentMIF(
        nuclear_channel='DAPI',
        cytoplasm_channel='CD45',  # Or other membrane/cytoplasmic marker
        model='mesmer',
        image_resolution=0.377,  # Microns/pixel
        compartment='whole-cell'  # 'nuclear', 'cytoplasm', or 'whole-cell'
    ),

    # 3. Quantify marker expression for each cell
    QuantifyMIF(
...
Integrate data from multi-cycle CODEX acquisitions into a single multi-channel image:

```python
from pathml.preprocessing import CollapseRunsCODEX

transform = CollapseRunsCODEX(
    z_slice=2,  # Which z-plane to extract (0-indexed)
    run_order=[0, 1, 2, 3],  # Acquisition cycle order
...
**Parameters:**
- `z_slice`: Which focal plane to extract from z-stack (typically middle slice)
- `run_order`: Cycle order; `None` for auto-detection
- `method`: How to merge channels from multiple cycles ('max', 'mean', 'median')
- `background_subtract`: Whether to subtract background fluorescence

**Output:** Single multi-channel image containing all markers (H, W, C)
...
transform = SegmentMIF(
    ...
    cytoplasm_channel='CD45',  # Cytoplasmic/membrane marker (required)
    model='mesmer',  # DeepCell Mesmer model
    image_resolution=0.377,  # Microns/pixel (critical for accuracy)
    compartment='whole-cell',  # Segmentation output
    min_cell_size=50,  # Minimum cell pixel size
    max_cell_size=1000  # Maximum cell pixel size
)
```

**Selecting cytoplasm channel:**
- **CD45**: Pan-leukocyte marker (suitable for immune-cell-rich tissues)
- **panCK**: Pan-cytokeratin (suitable for epithelial tissues)
- **CD298/b2m**: Generic membrane marker
- **Combination**: Average of multiple membrane markers

**Compartment options:**
- `'whole-cell'` - Full cell segmentation (nuclear + cytoplasmic)
- `'nuclear'` - Nuclear-only segmentation
- `'cytoplasm'` - Cytoplasmic portion only
...
**Output:** AnnData object containing:
...
Process multiple CODEX slides into a unified AnnData object:
...
# Calculate spatial neighbor enrichment
...
merfish_pipeline = Pipeline([
    ...
    SegmentMIF(
        nuclear_channel='DAPI',
        cytoplasm_channel='polyT',  # Use poly(T) staining for cell boundaries
        model='mesmer'
    ),
...
# Output: AnnData containing gene counts per cell
...
Efficiently process large multiparametric datasets:
...
def export_to_seurat(adata, output_file):
    ...
    # Read in R using: library(Seurat); ReadH5AD(output_file)
```

## Best Practices

1. **Channel selection for segmentation:**
   - Use the brightest, most consistent nuclear marker (typically DAPI).
   - Select membrane/cytoplasmic marker based on tissue type.
   - Test multiple options to optimize segmentation.

2. **Background subtraction:**
   - Apply background subtraction before quantification to reduce autofluorescence.
...
3. **Quality control:**
   ...
   - Verify marker expression ranges.

4. **Cell type annotation:**
   - Start with canonical markers (CD3, CD20, panCK).
   - Use multiple markers for robust classification.
   - Consider unsupervised clustering to discover new populations.

5. **Spatial analysis:**
   - Consider tissue structure (epithelial, stromal, etc.).
   - Account for local density when interpreting interactions.
   - Use permutation tests for significance assessment.

6. **Batch effects:**
   - Include batch information in `AnnData.obs`.
   - Apply batch correction when combining multiple experiments.
   - Use batch-colored UMAP to visualize batch effects.

## Common Issues and Solutions

**Problem: Poor segmentation quality**
- Check that nuclear and cytoplasmic channels are correctly specified.
- Adjust `image_resolution` parameter to match actual resolution.
- Try different cytoplasmic markers.
- Manually adjust min/max cell size parameters.

**Problem: Low marker intensity**
- Check for artifacts from over-aggressive background subtraction.
- Verify channel names match actual channels.
- Check for technical issues in raw images (out of focus, exposure).

**Problem: Cell type annotation not as expected**
- Adjust marker thresholds (too high or too low).
- Visualize marker distributions to set data-driven thresholds.
- Check for antibody specificity issues.

**Problem: Spatial analysis shows no significant interactions**
- Increase neighborhood radius.
- Check that cell counts for each type are sufficient.
- Verify spatial coordinates are properly scaled.
