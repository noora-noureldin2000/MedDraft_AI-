...
Load Neuropixels recording data from various formats.
...
Run the complete analysis pipeline. Returns a dictionary containing all results.
...
Detect and return a list of bad channel IDs.
...
Check for drift in the recording data. Returns drift statistics.
...
- `'kilosort3'`: Older version, requires MATLAB
- `'spykingcircus2'`: CPU-based alternative
- `'mountainsort5'`: Fast, suitable for short recordings
...
Create a SortingAnalyzer for postprocessing.
...
Complete postprocessing. Returns (analyzer, metrics).
...
- `presence_ratio`: Recording presence ratio
...
- `sliding_rp_violation`: Sliding window refractory period violation
...
Apply automatic curation. Returns {unit_id: label}.
...
Filter sorting results to keep only specified labels.
...
Generate a visual report for AI analysis.

Returns:
- `'image_path'`: Path to the saved plot
...
Analyze a unit using a vision-language model.

**Task Types:**
- `'quality_assessment'`: Classify as good/mua/noise
- `'merge_candidate'`: Check if units should be merged
...
Plot a complete unit summary.

## SpikeInterface Integration

All neuropixels_analysis functions support SpikeInterface objects:

```python
import spikeinterface.full as si
import neuropixels_analysis as npa

# SpikeInterface recording objects can be used directly in npa functions
recording = si.read_spikeglx('/path/')
rec = npa.preprocess(recording)

# For advanced usage, SpikeInterface can be accessed directly
...
- `freq_min`: High-pass cutoff frequency (Hz)
- `freq_max`: Low-pass cutoff frequency (Hz)
- `n_jobs`: Number of parallel jobs (-1 = use all cores)

### Sorting parameters
- `output_folder`: Path to save results
- `sorter_params`: Dictionary of specific sorter parameters

### Quality metric thresholds
- `snr_threshold`: SNR cutoff (typically 5)
- `isi_threshold`: ISI violations ratio cutoff (typically 0.01)
