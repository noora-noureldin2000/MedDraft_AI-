# Post-processing and Analysis Reference

A comprehensive guide to quality metrics, visualization, and analysis of sorted Neuropixels data.

## Sorting Analyzer

`SortingAnalyzer` is the core object for post-processing.
...
analyzer.compute('principal_components')  # PCA principal component analysis
analyzer.compute('spike_amplitudes')    # Amplitude of each spike
analyzer.compute('correlograms')        # Auto/cross-correlograms
analyzer.compute('unit_locations')      # Unit locations
analyzer.compute('spike_locations')     # Position of each spike
analyzer.compute('template_similarity') # Template similarity matrix
analyzer.compute('quality_metrics')     # Quality metrics

# Or compute multiple items at once
analyzer.compute([
...
| `presence_ratio` | Fraction of recording time containing spikes | > 0.9 |
| `firing_rate` | Spikes per second | 0.1-50 Hz |
| `amplitude_cutoff` | Estimated fraction of missing spikes | < 0.1 |
...
| `sliding_rp_violation` | Sliding refractory period violation | < 0.05 |
...
| `silhouette_score` | Cluster silhouette score | > 0.5 |
...
print(f"Number of good units: {len(good_units)}/{len(qm)}")
```
...
# Get waveforms for a specific unit
...
# Cosine similarity matrix between templates
```
...
print(locations)  # x, y coordinates for each unit
```
...
- `'center_of_mass'` - Fast, lower accuracy
- `'monopolar_triangulation'` - More accurate, slower
- `'grid_convolution'` - A good balance
...
# Auto-correlogram of unit i: correlograms[i, i, :]
# Cross-correlogram of units i and j: correlograms[i, j, :]
```
...
print(f"LFP sampling frequency: {lfp.get_sampling_frequency()} Hz")
```
...
# Then run: phy template-gui phy_export/params.py
```
