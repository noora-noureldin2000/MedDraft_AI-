...
A complete step-by-step guide for Neuropixels recording analysis, from raw data to curated units.

## Overview

This reference document records the complete analysis pipeline:
...
# Or use our convenient wrapper function
...
Suitable for recordings with strong artifacts:

```python
from ibldsp.voltage import decompress_destripe_cbin

# IBL destriping (highly effective)
...
```python
# Save for reuse (accelerate iterations)
...
rec_corrected = npa.correct_motion(
    rec_preprocessed,
    preset='nonrigid_accurate',  # or use 'kilosort_like' for speed
)
...
```python
# Create sorting analyzer (the core object for all postprocessing)
...
| `isi_violations_ratio` | < 0.01 | Refractory period violation ratio |
| `presence_ratio` | > 0.9 | Proportion of recording duration containing spikes |
| `amplitude_cutoff` | < 0.1 | Estimated false negative rate |
...
# Visual analysis of uncertain units
...
2. **Save preprocessed data** to avoid redundant computations.
3. For critical experiments, **compare results from multiple sorters**.
4. **Manually review uncertain units**—do not blindly trust automatic curation.
5. **Document your parameters** to ensure experimental reproducibility.
6. Kilosort4 **uses GPU** (10-50x faster than CPU alternatives).