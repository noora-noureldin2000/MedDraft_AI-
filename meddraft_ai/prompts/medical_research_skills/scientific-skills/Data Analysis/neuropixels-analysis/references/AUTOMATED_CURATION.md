# Automated Curation Reference Guide

A guide to using Bombcell, UnitRefine, and other tools for automated spike sorting curation.

## Why Automated Curation?

Manual curation has the following drawbacks:
- **Time-consuming**: Takes hours per recording session.
- **Subjectivity**: Variability between different raters.
- **Non-reproducible**: Difficult to standardize.

Automated tools provide consistent and reproducible quality classification.

## Available Tools

| Tool | Classification Type | Language | Integration |
|------|---------------|----------|-------------|
| **Bombcell** | 4 classes (Single-unit/Multi-unit/Noise/Non-somatic) | Python/MATLAB | SpikeInterface, Phy |
| **UnitRefine** | Machine learning-based | Python | SpikeInterface |
| **SpikeInterface QM** | Threshold-based | Python | Native support |
...
1. **Single somatic units** - Well-isolated single neurons (Single-unit)
2. **Multi-unit activity (MUA)** - Mixed neuronal signals (Multi-unit)
3. **Noise** - Non-neuronal artifacts (Noise)
4. **Non-somatic** - Axonal or dendritic signals (Non-somatic)
...
# Load sorting data (Kilosort output)
...
# Export to Phy format (Bombcell can read this format)
...
Bombcell calculates specific classification metrics:

| Metric | Description | Use Case |
|--------|-------------|----------|
| `peak_trough_ratio` | Waveform shape | Somatic vs Non-somatic |
| `spatial_decay` | Amplitude decay across channels | Noise detection |
| `refractory_period_violations` | ISI violations | Single-unit vs Multi-unit |
...
## SpikeInterface Automated Curation
...
### Using SpikeInterface Curation Modules
...
# Wrap sorting results for curation
...
# Find highly similar unit pairs
import numpy as np
threshold = 0.9
similar_pairs = np.argwhere(similarity > threshold)
# Merge unit pairs (use with caution - usually requires manual inspection)
...
Track the same neurons across different recording days.
...
# Returns a DataFrame of unit IDs matched across sessions
...
Combining automated and manual curation:
...
# Merge auto + manual labels for final results
```
...
| **SpikeInterface QM** | Fast, reproducible | Only supports simple thresholds |
| **Bombcell** | Multi-class, validated | Requires waveform extraction |
| **UnitRefine** | ML-based, learns from data | Requires training data |

## Best Practices

1. **Always visualize** - Don't blindly trust automated results.
2. **Document thresholds** - Record the exact parameters used.
3. **Validate** - Compare automated vs manual curation on a subset.
