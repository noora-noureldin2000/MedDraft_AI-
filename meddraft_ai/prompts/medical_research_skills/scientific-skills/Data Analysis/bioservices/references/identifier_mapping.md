...
This document provides detailed information on how to use BioServices to convert identifiers between different biological databases.
...
Biological databases use different identifier systems. Cross-referencing requires mapping between these systems. BioServices provides several methods:

1. **UniProt Mapping**: Comprehensive protein/gene ID conversion
2. **UniChem**: Compound ID mapping
3. **KEGG**: Cross-references built into entries
...
```python
# Mapping multiple IDs (comma-separated)
...
UniProt supports mapping between over 100 database pairs. Key ones include:
...
### Full List of Database Codes

To get the full, up-to-date list:
...
# Common patterns:
# - Source databases usually end with the source database name
# - UniProtKB uses "UniProtKB_AC-ID" or "UniProtKB"
# - Most other databases use their standard abbreviations
```

### Common Database Code Reference

**Gene/Protein Identifiers:**
- `UniProtKB_AC-ID`: UniProt Accession/ID
- `UniProtKB`: UniProt Accession
- `KEGG`: KEGG Gene ID (e.g., hsa:7535)
- `GeneID`: NCBI Gene (Entrez) ID
- `Ensembl`: Ensembl Gene ID
- `Ensembl_Protein`: Ensembl Protein ID
- `Ensembl_Transcript`: Ensembl Transcript ID
- `RefSeq_Protein`: RefSeq Protein ID (NP_)
- `RefSeq_Nucleotide`: RefSeq Nucleotide ID (NM_)

**Gene Nomenclature:**
- `HGNC`: HGNC (Human Gene Nomenclature Committee)
- `MGI`: MGI (Mouse Genome Informatics)
- `RGD`: RGD (Rat Genome Database)
- `SGD`: SGD (Saccharomyces Genome Database)
- `FlyBase`: FlyBase (Fruit fly database)
- `WormBase`: WormBase (Nematode database)
...
- `SUPFAM`: SUPFAM (Superfamilies)
- `PROSITE`: PROSITE (Protein motifs)

**Pathways & Networks:**
- `Reactome`: Reactome Pathways
- `BioCyc`: BioCyc Pathways
- `PathwayCommons`: Pathway Commons Database
- `STRING`: STRING (Protein-protein network)
- `BioGRID`: BioGRID (Interaction database)
...
UniChem focuses on mapping compound identifiers across different databases.
...
### Extracting Database Links from KEGG Entries
...
KEGG Gene IDs are formatted as `organism:gene_id`:
...
def compound_name_to_ids(compound_name):
    """Search for compound and get all database IDs."""
    k = KEGG()
...
def safe_batch_mapping(ids, from_db, to_db, chunk_size=100):
    ...
    for i in range(0, len(ids), chunk_size):
        ...
        try:
            ...
            # Try to handle individual IDs from failed chunk
...
Sometimes you need to map through an intermediate database:

```python
from bioservices import UniProt

def multi_hop_mapping(gene_symbol, organism="9606"):
    """Gene Symbol → UniProt → KEGG → Pathway."""
    u = UniProt()
    k = KEGG()
...
### Issue 4: Species Ambiguity

**Symptom:** Gene symbol maps to multiple species

**Solution:** Always specify species in search

```python
# Not recommended: Ambiguous
result = u.search("gene:TP53")  # Many species have TP53
...
**Symptom:** Old database IDs fail to map

**Solution:** Update to current ID first
...
for line in entry.split("\n"):
    if line.startswith("AC"):
        print(line)  # Show primary and secondary accessions
```
...
5. **Specify species whenever possible** to avoid ambiguity
...
For complete running examples, see:
- `scripts/batch_id_converter.py`: Command-line batch conversion tool
- `workflow_patterns.md`: Patterns for integration into larger workflows