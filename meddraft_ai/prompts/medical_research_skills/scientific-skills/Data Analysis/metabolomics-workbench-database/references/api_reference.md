# Metabolomics Workbench REST API Reference Guide
...
- **input_item**: Identifier type or search parameter
- **input_value**: The specific value to search for
- **output_item**: The data to return (e.g., all, name, summary)
- **output_format**: json or txt (defaults to json if omitted)
...
Get metabolite structure and identification data.

### Input Items

| Input Item | Description | Example |
|------------|-------------|---------|
| `regno` | Metabolomics Workbench registration number | 11 |
...
| `hmdb_id` | Human Metabolome Database (HMDB) ID | HMDB0000122 |
...
# Download structure image in PNG format
...
Access metabolomics study metadata and experimental results.
...
| `institute` | Institution name | UCSD |
| `last_name` | Researcher's last name | Smith |
| `metabolite_id` | Metabolite registration number | 11 |
...
| `mwtab` | Complete study data in mwTab format |
| `number_of_metabolites` | Number of measured metabolites |
...
| `all` | Get all RefMet entries | (no value needed) |
...
RefMet provides four levels of structural resolution:

1. **Super Class**: The broadest classification (e.g., "Organic compounds", "Lipids")
2. **Main Class**: The primary biochemical category (e.g., "Fatty acids", "Carbohydrates")
3. **Sub Class**: More specific subdivisions (e.g., "Monosaccharides", "Amino acids")
4. **Individual Metabolite**: Specific compounds with standardized names

## Context 4: MetStat

Filter studies by analysis and biological parameters using semicolon-separated format.
...
| 3 | Chromatography | HILIC, RP (reversed phase), GC, IC |
...
```bash
# Human blood diabetes study with LC-MS HILIC positive mode
...
Perform mass spectrometry precursor ion search by m/z value.
...
| `M+NH4` | Ammonium adduct | Common in ammonium-containing systems |
| `M+2H` | Double protonation | Multiply charged ions |
| `M+H-H2O` | Dehydrated protonation | Loss of water molecule |
| `M+2Na-H` | Disodium minus hydrogen | Multiple sodium ions |
| `M+CH3OH+H` | Methanol adduct | Methanol in mobile phase |
| `M+ACN+H` | Acetonitrile adduct | ACN in mobile phase |
| `M+ACN+Na` | ACN + sodium | ACN and sodium ions |
...
| `M+Cl` | Chloride adduct | Chloride-containing mobile phase |
| `M+FA-H` | Formate adduct | Formic acid in mobile phase |
| `M+HAc-H` | Acetate adduct | Acetic acid in mobile phase |
| `M-H-H2O` | Deprotonated minus water | Loss of water molecule |
| `M-2H` | Double deprotonation | Multiply charged ions |
| `M+Na-2H` | Sodium minus double proton | Mixed charge states |

#### Neutral

| Adduct | Description |
|--------|-------------|
| `M` | Neutral molecule | Direct ionization methods |
...
| `map_location` | Genetic map location |
| `summary` | Gene description |
| `taxid` | Taxonomy ID |
| `species` | Species short name |
| `species_long` | Species full name |
...
Get protein sequence and annotation data.
...
| `is_identical_to` | Identical sequence |
...
As of 2025, the Metabolomics Workbench REST API does not enforce strict rate limiting for reasonable use. However, best practices include:

- Setting delays between batch requests
- Caching frequently accessed reference data
- Using appropriate batch sizes for large-scale queries

## Additional Resources

- **Interactive REST URL Builder**: https://www.metabolomicsworkbench.org/tools/mw_rest.php
...
# 4. Perform m/z search to identify compounds
mz_value = 180.06
