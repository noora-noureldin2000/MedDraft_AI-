...
This document introduces the fundamental concepts and building blocks of LaminDB: Artifacts, Records, Runs, Transforms, Features, and data lineage tracking.

## Artifacts

Artifacts represent datasets in various formats (DataFrames, AnnData, SpatialData, Parquet, Zarr, etc.). They are the primary data objects in LaminDB.
...
These concepts capture computational lineage. **Transform** represents a reusable analysis step (notebook, script, or function), while **Run** records a specific execution instance.
...
# Start tracking (at notebook/script start)
ln.track()
...
# End tracking (at notebook/script end)
ln.finish()
```
...
Features define typed metadata fields used for validation and querying. They enable structured annotation and search.
...
### Adding Feature Annotations to Artifacts
...
- **Source code:** Script/notebook content and git commits
- **Environment:** Python packages and their versions
- **Input Artifacts:** Data loaded during execution
- **Output Artifacts:** Data created during execution
- **Execution metadata:** Timestamp, user, parameters
- **Computational dependencies:** Relationships between Transforms

### Viewing Lineage

```python
# Visualize complete lineage graph
artifact.view_lineage()
...
artifact.run              # Run instance that created it
artifact.run.transform    # Transform (code) used
...
LaminDB automatically manages artifact versions when source data or code changes.
...
1. **Use meaningful keys:** Organize keys hierarchically (e.g., `project/experiment/sample.h5ad`).
...
4. **Define features early:** Create feature registry before annotating.
5. **Use typed features:** Specify dtype for better validation.
6. **Leverage versioning:** Don't create new keys for minor modifications.
7. **Document transforms:** Add docstrings to tracked functions.
8. **Set projects:** Group related work for organization and access control.
