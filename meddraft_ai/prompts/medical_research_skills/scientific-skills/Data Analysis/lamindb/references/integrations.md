...
This document introduces LaminDB integrations with workflow managers, MLOps platforms, visualization tools, and other third-party systems.

## Overview

LaminDB supports extensive integrations across data storage, compute workflows, machine learning platforms, and visualization tools, enabling seamless incorporation into existing data science and bioinformatics pipelines.
...
Supports MinIO, Cloudflare R2, and other S3-compatible endpoints:
...
```python
# Install GCP extra dependencies
...
# Redun automatically tracks lineage outside of LaminDB
```
...
Scalable array storage supporting cellxgene:
...
# Query using DuckDB (without loading entire file)
path = artifact.cache()
...
# Scripts are tracked along with git commit history
...
transform.source_code  # Shows code for corresponding git commit
...
Sync with Benchling registry (requires team/enterprise plan):

```python
# Configure Benchling connection (contact LaminDB team)
# Sync schema and data from Benchling registry
...
Export datasets in Croissant metadata format:
...
# Export Croissant metadata (requires additional config)
# Enable dataset discovery and interoperability
```

## Integration Best Practices

1. **Track consistently:** Use `ln.track()` in all integrated workflows
2. **Link IDs:** Store external system IDs (e.g., W&B run ID, MLflow experiment ID) as features
3. **Centralize data:** Use LaminDB as the single source of truth for data artifacts
4. **Sync params:** Log parameters to both LaminDB and ML platforms
5. **Coordinate versioning:** Keep code (git), data (LaminDB), and experiments (ML platform) in sync
6. **Cache strategically:** Configure appropriate cache locations for cloud storage
7. **Use feature sets:** Link Bionty ontology terms to artifacts
8. **Document integrations:** Add descriptions to explain integration context
9. **Test incrementally:** Validate integrations with small datasets first
10. **Monitor lineage:** Use `view_lineage()` to ensure integration tracking works

## Troubleshooting

**Problem: Cannot find S3 credentials**
```bash
...
**Problem: MLflow artifacts not syncing**
```python
# Explicitly save to both systems
