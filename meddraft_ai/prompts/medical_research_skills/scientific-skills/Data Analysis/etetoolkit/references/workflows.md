# Common Workflows for ETE Toolkit

This document provides complete workflows for performing common tasks using the ETE Toolkit.
...
# Get a subtree rooted at a specific node
node = tree.search_nodes(name="MyNode")[0]
subtree = node.copy()

# Save the subtree to a file
subtree.write(outfile="subtree.nw", format=1)

# Extract a monophyletic clade
...
### Pruning a Tree to Specific Taxa
...
# Save in a format that includes custom features (NHX format)
...
# Collapse a node (remove the node but keep its children)
node_to_collapse = tree & "low_support_node"
node_to_collapse.delete()

# Add a new species to an existing clade
...
### Complete Gene Tree Analysis with Sequence Alignment

```python
from ete3 import PhyloTree

# Load gene tree and link to alignment
...
# Option 1: Species overlap algorithm (no species tree required)
...
# Find all orthologs for a query gene
...
for event in events:
    if query_gene in event.in_seqs:
        if event.etype == "S":  # Speciation event
            orthologs.extend([s for s in event.out_seqs if s != query_gene])
        elif event.etype == "D":  # Duplication event
...
# Split into independent gene families
...
# Get all monophyletic clades of a certain type
# First annotate leaves
for leaf in tree:
...
# Find the most similar pair of trees
min_dist = float('inf')
best_pair = None
...
# Get reference tree (the first one)
ref_tree = bootstrap_trees[0].copy()

# Count bipartition occurrences
bipartition_counts = {}
...
### Building Species Trees from the NCBI Taxonomy Database
...
### Annotating Existing Trees with NCBI Taxonomy
...
### Querying the NCBI Taxonomy Database
...
# Load a cluster tree with a data matrix
...
for metric in metrics:
    ...
    for node in tree.traverse():
        if not node.is_leaf():
            ...
            # Positive silhouette = well clustered
            # Negative silhouette = poorly clustered
...
### Adding Faces (Decorative Elements) to Nodes
...
ts = TreeStyle()
ts.layout_fn = layout
ts.show_leaf_name = False  # We add custom names manually
...
ts.arc_start = 0  # Start angle
ts.arc_span = 360  # Span angle (full circle)
ts.show_leaf_name = True
...
# Modifications are kept after closing
tree.show()

# Changes made in the GUI can be saved
tree.write(outfile="modified_tree.nw")
```
...
# 6. Visualize and mark evolutionary events
def layout(node):
