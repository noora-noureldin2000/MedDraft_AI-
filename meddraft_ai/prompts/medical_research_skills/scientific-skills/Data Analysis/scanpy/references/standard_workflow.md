# Scanpy Single-Cell Analysis Standard Workflow
...
sc.settings.verbosity = 3  # Log level: error (0), warning (1), info (2), hint (3)
...
```python
# Regress out total counts and mitochondrial gene percentage
...
# Visualize specific marker genes
...
```python
# Compare different conditions within cell types
...
- **QC thresholds**: `min_genes`, `min_cells`, `pct_counts_mt` - depends on data quality
- **Normalization target**: Usually 1e4, but can be adjusted
- **HVG parameters**: Affects strictness of feature selection
- **PCA components**: Check variance contribution plot to determine optimal number
- **Clustering resolution**: Higher values yield more clusters (typically 0.4-1.2)
...
5. Use known marker genes to validate cell type annotations
