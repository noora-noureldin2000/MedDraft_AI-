...
## Data Reading and Writing (sc.read_*)
...
sc.read_visium(path)                        # Read Visium spatial transcriptomics data
```

### Writing Functions

```python
adata.write_h5ad(filename)                  # Write as h5ad format
adata.write_csvs(dirname)                  # Write as CSV files
adata.write_loom(filename)                 # Write as loom format
adata.write_zarr(filename)                 # Write as zarr format
```
...
```python
sc.tl.ingest(adata, adata_ref)                   # Map to reference dataset
...
sc.pl.diffmap(adata, color='leiden')             # Diffusion map visualization
```
...
```python
sc.pl.paga(adata, color='leiden')                # PAGA plot
sc.pl.dpt_timeseries(adata)                      # DPT time series
```
...
### Color Parameters
- `color`: Variable to color by (gene name, obs column name)
- `use_raw`: Whether to use adata's `.raw` attribute
...
### Save Parameters
- `save`: Filename for saving image
...
sc.settings.autoshow = False           # Don't automatically show plots
sc.settings.autosave = True            # Automatically save plots
