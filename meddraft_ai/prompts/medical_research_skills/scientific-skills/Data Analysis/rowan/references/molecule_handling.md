...
Rowan uses the `stjames` library for molecular representation. The `stjames.Molecule` class provides a unified interface for creating molecules from various sources and accessing molecular properties.

## Table of Contents

1. [Creating Molecules](#creating-molecules)
2. [Molecular Properties](#molecular-properties)
...
Extended XYZ supports additional properties like forces and unit cell parameters.
...
### Calculated Properties (after calculation)
...
### Thermodynamic Properties (after frequency calculation)
...
# Using positive domain (0 to 360)
...
## Electronic Reasonability Checks

The `stjames.Molecule` class validates that charge and multiplicity are consistent with the electron count:
...
- Electron count = sum(atomic_numbers) - charge
