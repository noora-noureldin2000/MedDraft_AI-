...
The `Workflow` class represents a submitted computational task.
...
# Delete only result data (keep metadata)
...
rowan.submit_bde_workflow(
    initial_molecule: Molecule,
    bond_indices: tuple,  # (atom1_index, atom2_index)
    name: str = None,
    folder_uuid: str = None,
    max_credits: float = None
)
...
rowan.submit_ts_search_workflow(
    initial_molecule: Molecule,  # Initial guess structure
    name: str = None,
    folder_uuid: str = None,
    max_credits: float = None
)
...
workflows = rowan.list_workflows(
    name: str = None,           # Filter by name (fuzzy match)
...
workflows = rowan.batch_submit_workflow(
    ...
    workflow_type: str,         # Type for all workflows
    workflow_data: dict = {},
    folder_uuid: str = None,
    max_credits: float = None
) -> list[Workflow]
...
| `weekly_credits` | float | Weekly allocated credits |
...
try:
    ...
    print(f"Rate limit triggered, retry after: {e.retry_after}")
```
