...
Clean up protein structure (remove water, artifacts, fix residues).
...
- Removing non-protein molecules (water, ligands, ions)
- Fixing missing atoms in residues
- Resolving alternate conformations
- Standardizing residue names
...
# Load structure data
protein.refresh()  # Fetch PDB data if not loaded
...
**Warning:** Deleting a folder removes all workflows within it!
...
**Warning:** Deleting a project removes all folders and workflows within it!
...
### Organizing Drug Discovery Projects
...
# Set credit limit for each workflow
workflow = rowan.submit_pka_workflow(
    initial_molecule=mol,
    name="pKa calculation",
    max_credits=10.0  # Fails if exceeds 10 credits
)
```
