...
# Get results (not loaded by default)
...
# Macroscopic pKa
strongest_acid = data['strongest_acid']  # Strongest acidic pKa
strongest_base = data['strongest_base']  # Strongest basic pKa (if applicable)
...
- Reference known compounds for context
...
- Log S -1 to -3: Moderate solubility
...
# Analyze conformational distribution
...
- Lowest energy conformer may not be the most abundant in solution
...
**Interpretation:**
- 0 imaginary frequencies = true minimum (stable structure)
...
# Docking score (more negative is better)
...
**Interpretation:**
- Vina scores for drug-like molecules typically range from -12 to -6 kcal/mol
- More negative = stronger predicted binding
- Ligand strain > 3 kcal/mol suggests binding mode may not be viable
...
- **Vinardo**: Updated parameters, typically more accurate
...
aggregate = data['aggregate_score']  # Overall score
...
| < 0.5 | Low | Requires experimental validation |
...
- Novel protein folds not included in training data
- Flexible or disordered regions
- Unusual ligands (large, charged, or structurally complex)
...
4. Validate with experimental data if possible
...
|----------|-------------------|
| pKa | Potentiometric titration, UV spectrophotometry |
| Solubility | Shake flask method, turbidimetry |
...
1. **pKa deviation > 2 units**: Check tautomers, ensure protonation state is correct.
2. **Positive docking score**: Ligand may not be suitable for that binding site.
3. **Optimization not converged**: Try different starting geometry.
4. **Strain energy too high**: Conformation may be wrong.
...
```python
# Download SDF file with all poses
workflow.download_sdf_file("poses.sdf")

# Download trajectory files (for MD)
