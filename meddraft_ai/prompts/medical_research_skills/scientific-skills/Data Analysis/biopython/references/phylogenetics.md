# Phylogenetic Analysis with Bio.Phylo
...
- **NEXUS** - Extended format containing additional data
...
# Read a single tree from a file
...
- **Clade** - A node in the tree (internal or terminal)
- **Terminal clades** - Leaf nodes/tips (taxa)
...
# Find all clades meeting a condition
...
# Tree depth (distance from root to the farthest leaf)
depths = tree.depths()
...
```python
# Find the common ancestor of two clades
...
# Find the common ancestor of multiple clades
...
def compare_trees(tree1, tree2):
    ...
    # Check if they contain the same taxa
    if taxa1 != taxa2:
...
```python
# Ladderize the tree (sort branches by size)
tree.ladderize()  # Ascending
tree.ladderize(reverse=True)  # Descending
```
...
# Note: Coloring branches directly requires custom matplotlib code
```

## Tree Construction

### Construction from Distance Matrix
...
# Build tree using Neighbor-Joining (NJ) method
tree = constructor.nj(dm)
```

### Construction from Multiple Sequence Alignment (MSA)
...
The phyloXML format supports rich annotations:
...
```python
# Add bootstrap support values to the tree
...
1. **Choose the appropriate file format** - Use Newick for simple trees and phyloXML for annotated trees.
2. **Validate tree topology** - Check for polytomies and negative branch lengths.
3. **Reroot correctly** - Use midpoint rooting or outgroup rooting.
4. **Handle bootstrap values** - Store them as clade confidence.
5. **Consider tree size** - Large trees may require special handling.
6. **Use tree copies** - Call `.copy()` before making modifications.
7. **Export publication-quality charts** - Use matplotlib for high-quality image output.
8. **Document the tree-building process** - Record the alignments and parameters used.
9. **Compare multiple trees** - Use consensus methods for bootstrap trees.
10. **Validate taxon names** - Ensure consistent naming across different files.

## Common Use Cases

### Building a Tree from Sequences
...
def extract_subtree(tree, taxa_list):
    """Extract a subtree containing specific taxa."""
    # Create a copy
    subtree = tree.copy()
