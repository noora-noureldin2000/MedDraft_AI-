...
GeoPandas provides plotting capabilities through integration with matplotlib.
...
Requires installation: `uv pip install mapclassify`
...
# Passing parameters to classification schemes
...
# Use a separate axis as a colorbar
...
# Use zorder to control layer order (higher values are on top)
...
# Change marker size based on column values
...
# Add north arrow and scale bar (requires extra packages)
# Refer to geopandas-plot or contextily for related features
...
Requires installation: `uv pip install folium`
...
GeoPandas supports pandas plotting types:
...
Requires installation: `uv pip install contextily`
...
# Other data sources: ctx.providers.CartoDB.Positron, ctx.providers.Stamen.Terrain

plt.show()
```

## Handling Map Projections with CartoPy

Requires installation: `uv pip install cartopy`

```python
import cartopy.crs as ccrs

# Create map with a specific projection
