...
```python
# Transpose AnnData object (swap obs and vars)
adata_T = adata.T
...
# Useful when data orientation is reversed
# Common in some file formats (genes as rows, cells as columns)
```
...
### Shallow Copy / View
```python
# View (no data copy, modifications affect original)
...
### Renaming Observations and Variables
```python
...
# Make names unique (appends suffix to duplicates)
...
```python
# Convert string columns to categorical (saves memory)
...
act_expression = adata.obs_vector('ACTB')  # If ACTB in var_names
```
...
### Normalize
```python
# Total count normalization (like CPM/TPM)
...
Views are lightweight references to data subsets without copying the underlying matrix:
...
# Modifying view data affects original object
# (Use with caution!)

# Convert view to independent copy
independent = view.copy()

# Force AnnData to be a copy instead of view
adata = adata.copy()
```
...
# Extract subset of highly variable genes
