...
The AnnData object stores a data matrix and its associated annotations, providing a flexible framework for managing experimental data and metadata.

## Core Components

### X (Data Matrix)
The primary (n_obs, n_vars) shaped data matrix storing experimental measurements.
...
Accessing data:
```python
# Get full matrix (careful with large datasets)
full_data = adata.X

# Get single observation
obs_data = adata.X[0, :]

# Get single variable across all observations
var_data = adata.X[:, 0]
```

### obs (Observation Annotations)
DataFrame storing metadata about observations (rows). Each row corresponds to an observation in X.
...
DataFrame storing metadata about variables (columns). Each row corresponds to a variable in X.
...
- `scaled`: Z-score scaled values for analysis
- `imputed`: Imputed data

### obsm (Multi-dimensional Observation Annotations)
Dictionary storing multi-dimensional arrays aligned with observations.
...
Dictionary storing multi-dimensional arrays aligned with variables.
...
Common obsp uses:
- `connectivities`: Cell-cell proximity graph
...
### raw (Raw Data Snapshot)
Optional attribute preserving the raw data matrix and variable annotations before filtering.
...
# Storage mode
is_view = adata.is_view  # True if view of another object
is_backed = adata.isbacked  # True if backed by disk storage
...
gene_expression = adata.obs_vector('ACTB')  # If ACTB in var_names
...
AnnData structure is designed for memory efficiency:
- Sparse matrices reduce footprint for sparse data
- Views avoid copying data when possible
- Backed mode supports working with data larger than memory
- Categorical annotations reduce footprint for discrete values

```python
# Convert strings to categories (memory efficient)
