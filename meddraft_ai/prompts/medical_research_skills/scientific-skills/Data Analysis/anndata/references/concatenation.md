...
Combining multiple AnnData objects along the observations or variables axis.

## Basic Concatenation

### Along Observations (Stacking Cells/Samples)
```python
...
### Along Variables (Stacking Genes/Features)
```python
# Create objects with same observations but different variables
adata1 = ad.AnnData(X=np.random.rand(100, 50))
...
Keeps only variables/observations present in all objects.
...
# Inner join: Keep only genes 10-49 (overlap)
adata_inner = ad.concat([adata1, adata2], join='inner')
print(adata_inner.n_vars)  # 40 genes (overlap)
```

### Outer Join (Union)
Keeps all variables/observations, filling missing values.

```python
# Outer join: Keep all genes
...
### Specifying Fill Value for Outer Join
```python
...
adata_combined = ad.concat(
    ...
    label='batch',  # Column name for label
...
```python
# If keys not provided, uses integer index
adata_combined = ad.concat(
    [adata1, adata2, adata3],
    label='dataset'
)
...
### merge=None (Default for obs concatenation)
Excludes metadata not on the concatenation axis.

```python
# When concatenating along obs, var metadata must match
adata1.var['gene_type'] = 'protein_coding'
adata2.var['gene_type'] = 'protein_coding'

# var is kept only if metadata is identical across all objects
adata_combined = ad.concat([adata1, adata2], merge=None)
```

### merge='same'
Keeps metadata columns that are identical across all objects.
...
# Make indices unique by appending batch key
adata_combined = ad.concat(
    [adata1, adata2],
    label='batch',
    keys=['batch1', 'batch2'],
    index_unique='_'  # Joiner for unique index
)
...
# Layers are automatically merged if present in all objects
...
## Merging Multidimensional Annotations
...
# obsm is merged along observation axis
adata_combined = ad.concat([adata1, adata2])
...
# By default, obsp is not merged (set pairwise=True to include)
adata_combined = ad.concat([adata1, adata2])
...
# Use merge='unique' for uns
adata_combined = ad.concat([adata1, adata2], uns_merge='unique')
# 'date' kept (same value), 'batch' might be excluded (different value)
```

## Lazy Concatenation (AnnCollection)

For huge datasets, use lazy concatenation without loading all data:
...
print(collection.obs.head())  # Metadata loaded, X not loaded
...
## Disk-Based Concatenation

For datasets that don't fit in memory, concatenate directly on disk:
...
adata_combined = ad.concat(
    ...
    keys=['rep1', 'rep2', 'rep3'],
    join='inner'  # Keep only genes detected in all runs
)
```

### Merging Experimental Batches
```python
...
### Merging Multi-modal Data
```python
# Different modalities of measurement (e.g., RNA + Protein)
adata_rna = ad.AnnData(X=np.random.rand(100, 2000))
adata_protein = ad.AnnData(X=np.random.rand(100, 50))

# Merge along variables to combine modalities
adata_multimodal = ad.concat([adata_rna, adata_protein], axis=1)

# Add label to distinguish modalities
adata_multimodal.var['modality'] = ['RNA'] * 2000 + ['protein'] * 50
```

## Best Practices

1. **Check Compatibility Before Merging**
```python
# Verify shapes compatible
...
- `inner`: When you need identical features across all samples (strictest).
...
- Consider using backed mode for the result.

5. **Handle Batch Effects**
Concatenation combines data but doesn't correct for batch effects. Apply batch correction after merging:
```python
