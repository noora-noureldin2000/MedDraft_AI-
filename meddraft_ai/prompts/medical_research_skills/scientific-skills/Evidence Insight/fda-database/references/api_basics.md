...
This reference guide provides comprehensive information on using the openFDA API, including authentication, rate limits, query syntax, and best practices.
...
**All requests must use HTTPS**. HTTP requests are not accepted and will result in failure.

## Authentication

### API Key Registration

While openFDA can be used without an API key, registering for a free API key is highly recommended to obtain higher rate limits.

**Registration**: Visit https://open.fda.gov/apis/authentication/ to register.

**Advantages of an API Key**:
- Higher rate limits (240 requests/minute, 120,000 requests/day)
- Better suited for production environment applications
- No additional cost

### Using Your API Key

You can include your API key in requests using one of the following two methods:
...
|--------|-------------------|------------------|
| **No API Key** | 240 per IP address | 1,000 per IP address |
...
- **Error Message**: Indicates that the rate limit has been exceeded

**Best Practice**: Implement an Exponential Backoff algorithm:
```python
import requests
import time

def query_with_rate_limit_handling(url, params, max_retries=3):
    """Query the API and automatically handle rate limits."""
...
For applications requiring higher limits, please contact the openFDA team via their website with detailed information about your use case.
...
Parameters are separated by the logical AND symbol (`&`).
...
#### AND Operator
Combine multiple conditions (both must be met):
```python
# Find adverse events for aspirin within Canada
params = {
...
#### OR Operator
Either condition met is sufficient (spaces implicitly represent OR):
```python
...
Exclude results:
```python
# Exclude events occurring in the US
params = {
...
#### Wildcards
Use an asterisk (`*`) for fuzzy matching:
```python
...
# Open-ended range
params = {
    "search": "patient.patientonsetage:[65+TO+*]"  # 65 years and older
}
```
...
```python
# Data containing patient age records
params = {
    "search": "_exists_:patient.patientonsetage"
}

# Data missing patient age records
params = {
...
Controls the number of results returned (1-1000, default is 1):
```python
params = {
    "search": "...",
    "limit": 100
...
Used for pagination, skipping the first N results:
```python
# Get results 101-200
params = {
    "search": "...",
...
def get_all_results(url, search_query, api_key, max_results=5000):
    """Fetch results through pagination."""
    all_results = []
    skip = 0
    limit = 100
    ...
    while len(all_results) < max_results:
        ...
        skip += limit
        time.sleep(0.25)  # Polite rate limit wait
...
Aggregate and count results by field (instead of returning individual records):
```python
# Count events by country/region
params = {
    "search": "patient.drug.medicinalproduct:aspirin",
    "count": "occurcountry"
}
...
Add the `.exact` suffix for exact phrase counting (especially important for multi-word fields):
```python
# Count exact reaction terms (instead of word-split counts)
params = {
    "search": "patient.drug.medicinalproduct:aspirin",
...
**With `.exact`**: Counts the full phrase.
...
- **meta**: Metadata about the query and results
  ...
  - `results.skip`: Number of results skipped
  - `results.limit`: Maximum results per page
  - `results.total`: Total number of matching results (may be an approximation for large datasets)
...
- `INVALID_QUERY`: Search query malformed (400)
...
```python
# Drug adverse events where the serious outcome is death
params = {
...
```python
# Search for drug names in multiple fields
params = {
...
**DO**:
- Use specific fields for searching
...
def safe_api_call(url, params):
    """Safely call the FDA API with comprehensive error handling."""
    try:
...
class FDACache:
    """Simple file cache for FDA API responses."""
...
def cached_api_call(url, params):
    """API call with caching mechanism."""
    # Check cache
...
Track and adhere to rate limits:
```python
import time
...
def rate_limited_request(url, params):
    """Request sending with rate limiting."""
...
```python
# Get the 10 most common reactions for a certain drug
params = {
    "search": "patient.drug.medicinalproduct:aspirin",
    "count": "patient.reaction.reactionmeddrapt.exact",
    "limit": 10
...
- **Status Page**: Check for API downtime and maintenance
