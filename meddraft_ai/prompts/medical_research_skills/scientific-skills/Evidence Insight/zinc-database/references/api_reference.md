# ZINC Database API Reference Guide

## Overview

This guide provides a complete technical reference for programmatic access to the ZINC database, covering API endpoints, query syntax, parameters, response formats, and advanced usage patterns for ZINC22, ZINC20, and legacy versions.

## Base URLs

### ZINC22 (Current Version)
- **CartBlanche22 API**: `https://cartblanche22.docking.org/`
- **Files Library**: `https://files.docking.org/zinc22/`
- **Main Website**: `https://zinc.docking.org/`

### ZINC20 (Maintenance)
- **API**: `https://zinc20.docking.org/`
- **Files Library**: `https://files.docking.org/zinc20/`
...
### 1. Retrieving Substances by ZINC ID
...
**Parameters**:
- `zinc_id` (Required): A single ZINC ID or a comma-separated list
...
```bash
# Create a file containing ZINC IDs (one per line or comma-separated)
...
- `dist` (Optional): Tanimoto distance threshold (0-10, Default: 0 = exact match)
...
**Distance Parameter Interpretation**:
- `dist=0`: Exact match
...
Query compounds using supplier catalog numbers.
...
Generate sets of random compounds, optionally filtered by chemical properties.

**Endpoint**: `/substance/random.txt`

**Parameters**:
- `count` (Optional): Number of compounds to retrieve (Default: 100, max depends on server)
- `subset` (Optional): Filter by predefined subsets (e.g., 'lead-like', 'drug-like', 'fragment')
- `output_fields` (Optional): Comma-separated field names
...
**Subset Definitions**:
- `fragment`: Molecular Weight (MW) < 250, suitable for fragment-based drug discovery
- `lead-like`: MW 250-350, LogP ≤ 3.5, Rotatable Bonds ≤ 7
- `drug-like`: MW 350-500, follows Lipinski's Rule of Five
- `lugs`: Large and Ultra-Good Subset (highly curated)

## Output Fields

### Available Fields

Use the `output_fields` parameter to customize the API response:
...
| `supplier_code` | Supplier catalog number | AB-1234567 |
...
| `hbd` | H-bond donors | 2 |
| `rotatable_bonds` | Number of rotatable bonds | 5 |

**Note**: Not all fields are available for all endpoints. Field availability depends on the database version and endpoint.

### Default Fields

If `output_fields` is not specified, the endpoint returns all available fields in TSV format.
...
ZINC organizes compounds into "tranches" based on molecular properties to facilitate efficient filtering and organization.
...
|--------------|----------------|
| `H00P010M250-0` | 0 H-bond donors, LogP=1.0, MW=250 Da, phase 0 |
| `H05P035M400-0` | 5 H-bond donors, LogP=3.5, MW=400 Da, phase 0 |
| `H02P-005M180-0` | 2 H-bond donors, LogP=-0.5, MW=180 Da, phase 0 |

### Reactivity Phases

| Phase | Description |
|-------|-------------|
| 0 | Non-reactive (Preferred for screening) |
...
def parse_tranche(tranche_str):
    """
    Parses a ZINC tranche code.

    Args:
        tranche_str: Tranche code (e.g., "H05P035M400-0")

    Returns:
        Dictionary containing h_donors, logp, mw, phase
    """
...
### Filtering by Tranche

Download specific tranches from the files library:
```bash
# Download all compounds in a specific tranche
wget https://files.docking.org/zinc22/H05/H05P035M400-0.db2.gz
```

## Files Library Access

### Directory Structure

ZINC22 3D structures are organized by H-bond donor hierarchy:
...
|-----------|--------|-------------|
| `.db2.gz` | DOCK Database | Compressed multi-conformer database for DOCK |
...
**Multiple Tranches** (using aria2c for parallel downloads):
```bash
...
**Recursive Download** (Use with caution - massive data volume):
```bash
...
def advanced_zinc_search(smiles=None, zinc_ids=None, dist=0,
    ...
    Args:
        ...
        dist: Similarity distance parameter (0-10)
...
Leverage tranche data to filter compounds by molecular properties:

```python
def filter_by_properties(df, mw_range=None, logp_range=None,
                        max_hbd=None, phase=0):
    """
    Filters a DataFrame based on molecular properties.

    Args:
        df: DataFrame with 'tranche' column
        mw_range: Tuple (min_mw, max_mw)
        logp_range: Tuple (min_logp, max_logp)
        max_hbd: Maximum number of H-bond donors
        phase: Reactivity phase (0 = non-reactive)
...
- **Avoid rapid-fire requests**: Maintain at least 1 second between queries
- **Use batch operations**: Query multiple ZINC IDs in a single request
- **Cache results**: Store frequently accessed data locally
- **Off-peak usage**: Perform large downloads during off-peak hours (UTC night/weekends)
...
def polite_zinc_query(query_func, *args, delay=1.0, **kwargs):
    """Wrapper function to add delay between queries."""
...
```bash
# 1. Download tranche file
wget https://files.docking.org/zinc22/H05/H05P035M400-0.db2.gz

# 2. Decompress
gunzip H05P035M400-0.db2.gz

# 3. Use directly in DOCK6
...
def process_zinc_results(zinc_df):
    """
    Process ZINC results using RDKit.

    Args:
        zinc_df: DataFrame with a 'smiles' column
...
**Issue**: Empty results or no results
- **Solution**: Check SMILES syntax, verify ZINC ID existence, try a broader similarity search.

**Issue**: Timeout errors
- **Solution**: Reduce the number of results, use batch queries, try during off-peak hours.

**Issue**: Invalid SMILES encoding
- **Solution**: URL-encode special characters (use `urllib.parse.quote()` in Python).

**Issue**: Tranche file not found
- **Solution**: Verify tranche code format, check the files library directory structure.
...
|---------|--------|--------|--------|
| Compound Count | 230M+ purchasable | Lead-focused | ~750M total |
| API | CartBlanche22 | Similar | REST-like |
| Tranches | Yes | Yes | Yes |
| 3D Structures | Yes | Yes | Yes |
| Status | Current, growing | Maintenance | Legacy |

### API Compatibility

Most query patterns are universal across versions, but URLs differ:
