...
Used when exploring available data without loading the expression matrix.
...
**Pattern: Count cells by conditions**
```python
cell_metadata = cellxgene_census.get_obs(
    census,
    "homo_sapiens",
...
Use `get_anndata()` when results fit in memory (typically < 100,000 cells).

**Pattern: Cell type query for a specific tissue**
```python
...
**Pattern: Specific multi-gene query**
```python
...
**Pattern: Specific disease query**
```python
adata = cellxgene_census.get_anndata(
    census=census,
    organism="Homo sapiens",
...
For queries exceeding available RAM, use `axis_query()`.
...
# Iteratively process X matrix in chunks
iterator = query.X("raw").tables()
for batch in iterator:
    # Process batch (a pyarrow.Table)
    # batch contains columns: soma_data, soma_dim_0, soma_dim_1
    process_batch(batch)
```
...
Use `experiment_dataloader()` to train models.

**Pattern: Create training data loader**
```python
...
### 1. Always Filter Primary Data
Always include `is_primary_data == True` unless specifically analyzing duplicate data:
```python
obs_value_filter="cell_type == 'B cell' and is_primary_data == True"
```

### 2. Specify Census Version
To ensure reproducibility of analysis, always specify the Census version:
```python
census = cellxgene_census.open_soma(census_version="2023-07-25")
```

### 3. Use Context Managers
Always use context managers to ensure resources are properly cleaned up:
```python
with cellxgene_census.open_soma() as census:
    # Your code
```
...
Minimize data transfer by selecting only the required metadata columns:
```python
obs_column_names=["cell_type", "tissue_general", "disease"]  # Instead of all columns
```

### 5. Check Dataset Presence Before Gene Queries
When analyzing specific genes, check which datasets measured those genes:
```python
...
### 6. Use tissue_general for Broad Queries
`tissue_general` provides coarser grouping than `tissue`, suitable for cross-tissue analysis:
```python
# Suitable for broad queries
...
Explore metadata first to understand available data before querying expression:
```python
# Step 1: Exploration
...
For large queries, check the estimated size before loading:
```python
...
Use ontology term IDs instead of free text whenever possible:
```python
# More reliable than using cell_type == 'B cell' across datasets
obs_value_filter="cell_type_ontology_term_id == 'CL:0000236'"
```

### 10. Batch Processing Mode
For systematic analysis across multiple conditions:
```python
...
1. **Not filtering is_primary_data**: Leads to double-counting cells.
2. **Loading too much data**: Metadata queries should be used first to estimate size.
3. **Not using context managers**: Can lead to resource leaks.
4. **Inconsistent versioning**: Not specifying a version leads to non-reproducible results.
5. **Broad query scope**: Start with focused queries and expand as needed.
6. **Ignoring dataset presence**: Some genes are not measured in all datasets.
7. **Count normalization errors**: Be aware of the differences between UMI and read counts.