# Datamol descriptors and visualization reference guide

## Descriptors module (`datamol.descriptors`)

The descriptors module provides utilities to compute molecular properties and descriptors.
...
Compute the proportion of aromatic atoms over heavy atoms.
- **Returns**: Float between 0 and 1
- **Use case**: Quantify aromatic character

#### `dm.descriptors.n_charged_atoms(mol)`
Count atoms that carry a non-zero formal charge.
- **Returns**: Integer count
- **Use case**: Charge distribution analysis

#### `dm.descriptors.n_rigid_bonds(mol)`
Count the number of non-rotatable bonds (neither single rotatable bonds nor ring bonds).
- **Returns**: Integer count
- **Use case**: Assess molecular rigidity/flexibility

#### `dm.descriptors.n_stereo_centers(mol)`
Count stereocenters (chiral centers).
- **Returns**: Integer count
- **Use case**: Stereochemistry analysis

#### `dm.descriptors.n_stereo_centers_unspecified(mol)`
Count stereocenters lacking explicit stereochemical specification.
- **Returns**: Integer count
- **Use case**: Identify incomplete stereochemical information

### Bulk descriptor computation

#### `dm.descriptors.compute_many_descriptors(mol, properties_fn=None, add_properties=True)`
Compute a set of molecular properties for a single molecule.
- **Parameters**:
  - `properties_fn`: Custom list of descriptor functions
  - `add_properties`: Whether to include additional computed properties
- **Returns**: Dict mapping descriptor names to values
...
Retrieve any RDKit descriptor function by name.
- **Parameter**: `name` — descriptor function name (for example 'MolWt', 'TPSA')
...
# filter BBB-permeable candidates by TPSA
...
The `viz` module provides tools to render molecules and conformers to images.
...
Generate an image grid from molecules.
- **Parameters**:
  ...
  - `use_svg`: Output SVG (True) or PNG (False, default)
...
- **Returns**: Image object (displayable in Jupyter) or saves to file
- **Examples**:
  ```python
  # basic grid
...
Display multiple conformers in a grid layout.
- **Parameters**:
  - `mol`: Molecule containing embedded conformers
  ...
  - `sync_views`: Synchronize 3D views in interactive displays (default: True)
  - `remove_hs`: Remove hydrogens for clearer depictions (default: True)
- **Returns**: Conformer visualization grid
- **Use case**: Compare conformer diversity
- **Example**:
  ```python
...
### Circular grid visualization

#### `dm.viz.circle_grid(center_mol, circle_mols, mol_size=200, circle_margin=50, act_mapper=None, ...)`
Create a concentric circular visualization centered on a reference molecule.
- **Parameters**:
  - `center_mol`: Molecule placed at the center
  - `circle_mols`: List of lists of molecules (each sublist represents a ring)
  - `mol_size`: Image size for each molecule
  - `circle_margin`: Spacing between rings (default: 50)
  - `act_mapper`: Activity mapping dict for color coding
- **Returns**: Circular grid image
- **Use cases**: Visualize molecular neighborhoods, SAR analysis, similarity networks
- **Example**:
  ```python
...
- **Use labels for clarity**: Always label molecules with SMILES, IDs, or activity values.
- **Align related molecules**: Use `align=True` in `to_image()` for SAR analysis.
- **Adjust grid size**: Set `n_cols` according to molecule count and display width.
- **Use SVG for publication**: Set `use_svg=True` for scalable vector graphics.
- **Highlight substructures**: Emphasize features with `highlight_atom` and `highlight_bond`.
- **Save large grids**: Use the `outfile` parameter to save instead of displaying in memory.