...
Static image export requires Kaleido to be installed:

```bash
uv pip install kaleido
```

Kaleido v1+ requires Chrome/Chromium to be installed on the system.
...
# Format is automatically inferred from the extension
...
```python
# Embed the full library file (approx. 5MB)
fig.write_html('chart.html', include_plotlyjs=True)

# Reference CDN (approx. 2KB, requires internet connection)
fig.write_html('chart.html', include_plotlyjs='cdn')

# Local reference (requires plotly.min.js in the same directory)
fig.write_html('chart.html', include_plotlyjs='directory')

# Exclude library (used for embedding charts into existing HTML that already loads Plotly.js)
...
```python
# Get only the div tag (without the full HTML structure)
...
Plotly charts automatically support:
...
# Hover mode
fig.update_layout(
    hovermode='closest'  # Options: 'x', 'y', 'closest', 'x unified', False
)
...
fig = px.scatter(
    df, x='x', y='y',
    hover_data={
        'extra_col': True,     # Show this column
        'x': ':.2f',           # Format this column
...
For web applications, use Dash or FigureWidget to handle clicks:

```python
# Use FigureWidget in Jupyter
...
fig = px.scatter(
    df, x='gdp', y='life_exp',
    animation_frame='year',     # Animate based on this column
...
config = {
    ...
    'modeBarButtonsToRemove': ['pan2d', 'lasso2d'],  # Remove specific buttons
    'toImageButtonOptions': {
        'format': 'png',
        'filename': 'custom_image',
        'height': 500,
...
- `responsive`: Responsive resizing