...
parse_csv_tokens <- function(value, arg_name) {
...
parse_years_arg <- function(value) {
  years <- suppressWarnings(as.numeric(parse_csv_tokens(value, "--years")))
  if (any(is.na(years)) || any(!is.finite(years)) || any(years <= 0))
    stop_skill("SKILL_INVALID_PARAMETER", "--years must contain positive numeric values")
  unique(years)
}

parse_color_list <- function(value) {
  parse_csv_tokens(value, "--colors")
}
...
save_session_info <- function(output_dir, metadata = list()) {
  ...
  lines <- c(
    "Model Calibration Curve Session Information",
    "=========================================",
