...
canonicalize_path <- function(path) {
  ...
  if (identical(prefix, "/")) {
    return(if (nzchar(suffix)) paste0("/", suffix) else "/")
  }
  if (nzchar(prefix)) {
    return(if (nzchar(suffix)) paste0(prefix, "/", suffix) else paste0(prefix, "/"))
  }
...
resolve_output_dir <- function(output_dir, skill_root) {
  ...
  candidate <- if (grepl("^([A-Za-z]:)?[/\\]", output_dir)) {
    output_dir
  } else {
    file.path(skill_root, output_dir)
  }
  ...
  if (!starts_with_path(normalized, skill_root)) {
    fail(
      ...
      sprintf("Output directory must stay inside the skill root: %s", skill_root)
    )
  }
