...
init_logging <- function(output_dir) invisible(TRUE)
...
fail <- function(code, message_text) stop(sprintf("%s: %s", code, message_text), call. = FALSE)

check_required_packages <- function(packages) {
  missing <- packages[!vapply(packages, requireNamespace, logical(1), quietly = TRUE)]
  if (length(missing) > 0) {
    fail("SKILL_PACKAGE_NOT_FOUND", sprintf("Missing required package(s): %s", paste(missing, collapse = ", ")))
  }
}
