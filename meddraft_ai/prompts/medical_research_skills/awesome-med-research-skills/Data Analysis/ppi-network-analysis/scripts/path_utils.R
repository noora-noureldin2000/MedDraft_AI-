get_skill_root <- function(script_dir) normalizePath(file.path(script_dir, ".."), winslash = "/", mustWork = TRUE)
...
starts_with_path <- function(path, root_path) {
  ...
  identical(normalized_path, normalized_root) || startsWith(normalized_path, paste0(normalized_root, "/"))
}

resolve_existing_input <- function(path, label) {
  if (is.null(path) || !nzchar(trimws(path))) fail("SKILL_INVALID_PARAMETER", sprintf("%s is required.", label))
  normalized <- normalizePath(trimws(path), winslash = "/", mustWork = FALSE)
  if (!file.exists(normalized)) fail("SKILL_FILE_NOT_FOUND", sprintf("%s does not exist: %s", label, path))
  normalizePath(normalized, winslash = "/", mustWork = TRUE)
}

resolve_input_dir <- function(path, label) {
  normalized <- resolve_existing_input(path, label)
  if (!dir.exists(normalized)) fail("SKILL_FILE_NOT_FOUND", sprintf("%s is not a directory: %s", label, path))
  normalized
}

resolve_output_dir <- function(output_dir, skill_root) {
  if (is.null(output_dir) || !nzchar(trimws(output_dir))) fail("SKILL_INVALID_PARAMETER", "--output_dir must not be empty.")
