...
source(file.path(script_dir, "string_cache.R"))
...
main <- function() {
  check_required_packages(c("optparse", "dplyr", "openxlsx", "igraph"))
  parser <- optparse::OptionParser(option_list = build_option_list(), description = "Build a local-cache PPI network and generate a reproducible PDF plot.")
  default_cache_dir <- normalizePath(file.path(script_dir, "..", "references", "string_cache"), winslash = "/", mustWork = FALSE)
  options <- validate_options(optparse::parse_args(parser), get_skill_root(script_dir), default_cache_dir)
  init_logging(options$output_dir)
  log_info("Starting ppi-network-analysis.")
