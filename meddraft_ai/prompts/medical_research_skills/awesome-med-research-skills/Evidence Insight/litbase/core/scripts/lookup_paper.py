"""
Paper metadata query tool.

Usage:
  python lookup_paper.py --doi "10.xxxx/xxxxx"
  python lookup_paper.py --title "The Effect of Identity Signaling..."
"""

import urllib.request
import urllib.parse
import json
import argparse
import sys
import os
import time

S2_BASE = "https://api.semanticscholar.org/graph/v1"

_cfg_path = os.path.join(os.path.dirname(__file__), "..", "config.json")
with open(_cfg_path, encoding="utf-8") as _f:
    _cfg = json.load(_f)

HEADERS = {
    "User-Agent": "LitBase/1.0",
    "x-api-key": _cfg.get("s2_api_key", ""),
}

PAPER_FIELDS = "title,year,citationCount,journal,authors,externalIds,abstract,publicationVenue"
AUTHOR_FIELDS = "name,hIndex,citationCount,affiliations"


def fetch(url, retries=3):
    for attempt in range(retries):
        try:
            req = urllib.request.Request(url, headers=HEADERS)
            with urllib.request.urlopen(req, timeout=15) as resp:
                return json.loads(resp.read())
        except urllib.error.HTTPError as e:
            if e.code == 429 and attempt < retries - 1:
                wait = 10 * (attempt + 1)
                print(f"  [Rate limited, retrying in {wait}s]")
                time.sleep(wait)
            elif e.code == 404:
                return None  # paper not in S2, normal fallback
            else:
                raise
        except Exception as e:
            print(f"  [Network error] {e}")
            if attempt == retries - 1:
                return None
    return None


def get_paper_by_doi(doi):
    url = f"{S2_BASE}/paper/DOI:{urllib.parse.quote(doi)}?fields={PAPER_FIELDS}"
    return fetch(url)


def get_paper_by_title(title):
    params = urllib.parse.urlencode({"query": title, "fields": PAPER_FIELDS, "limit": 1})
    url = f"{S2_BASE}/paper/search?{params}"
    data = fetch(url)
    if data and data.get("data"):
        return data["data"][0]
    return None


def get_author(author_id):
    url = f"{S2_BASE}/author/{author_id}?fields={AUTHOR_FIELDS}"
    return fetch(url)


def format_output(paper):
    lines = []

    title = paper.get("title", "N/A")
    year = paper.get("year", "N/A")
    citations = paper.get("citationCount", "N/A")

    venue = paper.get("publicationVenue") or paper.get("journal") or {}
    journal_name = venue.get("name", "N/A") if isinstance(venue, dict) else "N/A"

    doi = (paper.get("externalIds") or {}).get("DOI", "N/A")

    lines.append("=" * 60)
    lines.append(f"Title:    {title}")
    lines.append(f"Year:     {year}")
    lines.append(f"DOI:      {doi}")
    lines.append(f"Journal:  {journal_name}")
    lines.append(f"Citations: {citations}")
    lines.append("")

    authors = paper.get("authors", [])
    if authors:
        lines.append(f"Authors ({len(authors)} total):")
        for a in authors[:5]:
            author_id = a.get("authorId")
            name = a.get("name", "N/A")
            if author_id:
                try:
                    detail = get_author(author_id)
                    if detail:
                        h = detail.get("hIndex", "?")
                        c = detail.get("citationCount", "?")
                        aff = detail.get("affiliations", [])
                        aff_str = aff[0] if aff else "N/A"
                        lines.append(f"  - {name} | h-index: {h} | citations: {c} | affiliation: {aff_str}")
                    else:
                        lines.append(f"  - {name} [author query failed]")
                except Exception:
                    lines.append(f"  - {name}")
        if len(authors) > 5:
            lines.append(f"  ... {len(authors)} authors total")

    lines.append("=" * 60)
    return "\n".join(lines)


def main():
    parser = argparse.ArgumentParser(description="Query paper metadata from Semantic Scholar")
    parser.add_argument("--doi", help="Paper DOI")
    parser.add_argument("--title", help="Paper title (fuzzy search)")
    args = parser.parse_args()

    if not args.doi and not args.title:
        print("Please provide --doi or --title")
        sys.exit(1)

    try:
        if args.doi:
            paper = get_paper_by_doi(args.doi)
        else:
            paper = get_paper_by_title(args.title)

        if not paper:
            print("No matching paper found")
            sys.exit(1)

        print(format_output(paper))
    except Exception as e:
        print(f"Query failed: {e}")
        sys.exit(1)


if __name__ == "__main__":
    main()
