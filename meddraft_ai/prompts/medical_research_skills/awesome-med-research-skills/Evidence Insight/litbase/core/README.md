...
In Claude Code: File → Open Folder → select the `litbase` folder.
...
LitBase also works on Web Claude without any installation:
...
**Literature integrity guarantee**: every citation in the RFD comes from your confirmed reading list. LitBase never fabricates references, PMIDs, DOIs, or study findings.
