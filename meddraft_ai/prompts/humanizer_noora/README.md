...
2. Copy the `humanizer-Noora` directory into the Claude Code skills directory:
   ```bash
   cp -r ./writing-modules/humanizer-Noora ~/.claude/skills/humanizer-Noora
   ```
...
2. Copy the `humanizer-Noora` directory into the OpenCode skills directory:
   ```bash
   cp -r ./writing-modules/humanizer-Noora ~/.config/opencode/skills/humanizer-Noora
   ```
...
If you are using web interfaces like **ChatGPT** or the **Claude.ai web console**, you can copy the contents of the style guide at [humanizer-Noora.md](humanizer-Noora.md) and paste it before your text with this prompt:
